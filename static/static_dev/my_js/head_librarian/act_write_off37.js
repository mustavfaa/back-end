...
new Vue({
    ...
    computed: {
        ...
        filterEditions2() {
            let all = this.editionss.map(item => {
                ...
                item.style = false
                item.idx = ''
                const sameElem = this.editions_val.find(el => item.edition.id === el.edition.id && item.income === el.income);

                if (sameElem) {
                    item.edition = sameElem.edition
            ...
            if (this.klass || this.language || this.edition || this.subject) {
                ...
                if (this.edition) {
                    filter.e_name = this.edition.name;
                }
                if (this.subject) {
        ...
        SummEditions() {
            ...
            this.filterEditions2.map(item => {
                sum.amount += (Number(item.quantity)*Number(item.price));
            ...
            .then(response => {
                (this.invoices = response.data.invoices.map(item => {
                    ...
                    item.editions_val.map(ed => {
                        item.sum = item.sum + ed.price * ed.quantity;
                        return ed;
                    });
                    return item;
                })),
                    ...
                    resp = response.data.editions;

                _.each(this.editionss, function (e, i) {
                    let pk = e.edition;
                    ob = _.findWhere(resp, {id: pk});
                    e.edition = ob;
                    e.quantity = 0;
                    e.summ_spis = 0;
                    e.klass = ob.klass;
                    e.language = ob.language;
                    e.e_name = ob.name;
                    e.subject = ob.subject;
                    e.price = e.summ / e.has;
    ...
    methods: {
        ...
        setOst(response) {
            ...
            edsss = this.editionss;
            _.each(response.data.books, function (e, i) {
                a = 5;
                curr = e;
                _.each(edsss, function (el, it) {
                    if (el.edition.id === curr.edition) {
                        el.has = curr.has;
                        el.summ = curr.summ;
                        el.price = el.summ / el.has;
                    }
                })
        ...
        defGETValid(response) {
            if (response.data.errors) {
                ...
                axios.get(document.location.href + 'get_ost/').then(response => {
                    this.setOst(response)
                });
            }
            this.loading2 = false;
        },
...
Vue.component("EditionsList", {
    ...
    mounted() {
        if (this.datamodel.idx > 0) {
            ...
            this.income = this.datamodel.income
            this.income_type = this.datamodel.income_type
        }
    },

    methods: {

        QuantityChange(datamodel) {
            datamodel.summ_spis = parseFloat(datamodel.quantity) * parseFloat(datamodel.price)
        },
            ...
            if (this.datamodel.quantity > 0 ) {
                datas = {
                    create_edition_invoice: {
                        ...
                        amount: Number(this.datamodel.quantity) *  Number(this.datamodel.price),
                        price: Number(this.datamodel.price),
                         income: Number(this.datamodel.income),
                         income_type: Number(this.datamodel.income_type),
                        has: Number(this.datamodel.has),
                    }
                };
                console.log(datas)
                axios
